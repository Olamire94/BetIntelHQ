...
MIN_VALUE_EDGE = 5.0
MAX_TIPS_PER_RUN = 5
MIN_ODDS = 1.5 MAX_ODDS = 4.0 # ignore odds shorter than this (too short)
# ignore odds longer than this (too risky)
...
reasoning = (
"Fair probability " + str(round(fair_probs[i]*100, 1)) + "% vs implied "
+ str(round(decimal_to_implied_prob(best["price"])*100, 1)) + "% at "
+ best["bookmaker"] + ". Edge: +" + str(round(edge, 1)) + "%."
)
best_tip = {
"match": home + " vs " + away,
...
tips = await get_tips()
if not tips:
return "Daily Summary\n\nNo value bets found today. Markets look sharp - patience pay
...
lines.append("")
lines.append("Gamble responsibly. Past performance does not guarantee future results.")
return "\n".join(lines)
