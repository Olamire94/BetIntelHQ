...
"Value Bet Bot\n\n1 tip per day.\nScans at 6am EST for the best odds between 1.30 and
...
text += "\nScheduled for: " + scheduled_tip["send_time"].astimezone(EST).strftime("%H
...
summary = build_results_summary(tip_results, datetime.now(EST).strftime("%d %b await msg.edit_text(summary)
except Exception as e:
await msg.edit_text("Error: " + str(e))
%Y"))
...
"How It Works\n\n"
"Every day at 6am EST the bot scans all sports for the single best tip with odds
...
try:
tip_results = await get_results([sent_today["tip"]])
summary = build_results_summary(tip_results, datetime.now(EST).strftime("%d %b await ctx.bot.send_message(CHANNEL_ID, summary)
%Y"))
